...
export const eccEncrypt = async ({
  ...
  const jscu = getJscu();
  const ephemeralKeyPair = await jscu.pkc.generateKey(
    'EC', // ECC key pair
    {namedCurve: 'P-256'} // or 'P-384', 'P-521', 'P-256K'
  );
  const privateJwk = await ephemeralKeyPair.privateKey.export('jwk');
  const publicJwk = await ephemeralKeyPair.publicKey.export('jwk');

  const myKeyPair = await jscu.pkc.generateKey(
    'EC', // ECC key pair
    {namedCurve: 'P-256'} // or 'P-384', 'P-521', 'P-256K'
  );
  const myPrivateJwk = await myKeyPair.privateKey.export('jwk');
  const myPublicJwk = await myKeyPair.publicKey.export('jwk');

  const compact = await myKeyPair.publicKey.export('oct', {compact: true});
  console.log(compact.length);

  const sharedA = await ec.deriveSecret(publicJwk, myPrivateJwk);
  console.log(jseu.encoder.arrayBufferToHexString(sharedA));

  const sharedB = await ec.deriveSecret(myPublicJwk, privateJwk);
  console.log(jseu.encoder.arrayBufferToHexString(sharedB));

  console.log(sharedB.length);
};
