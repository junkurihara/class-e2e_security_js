...
\begin{itemize}
\item プロジェクトのGitHubリポジトリ\footnote[frame]{\url{https://github.com/junkurihara/slides-e2e-security-js/}}をClone\\
\begin{exampleblock}{}
\footnotesize
\$ \texttt{git clone https://github.com/junkurihara/slides-e2e-security-js.git}\\
\$ \texttt{cd sample02}
\end{exampleblock}
\item 依存パッケージのインストール
\begin{exampleblock}{}
...
今回はSSL接続可能な共有サーバを準備済 (\url{https://e2e.zettant.com/})\footnote[frame]{\alert{2020/1/25時点でサーバは停止している。モックサーバを利用のこと。}}。
...
\begin{center}
\includegraphics[width=\linewidth]{Figs/data_pbes.pdf}
\end{center} 
...
\begin{center}
\includegraphics[width=\linewidth]{Figs/data_hkdf.pdf}
\end{center} 
