...
以下の環境が前提:
\begin{itemize}
 \item Node.jsがインストール済 \footnote[frame]{npmに加えてyarnも使えるとなお良い (npm i -g yarn)}
...
\begin{exampleblock}{}
\footnotesize
\$ \texttt{git clone https://github.com/junkurihara/e2e-security-class}\\
\$ \texttt{cd e2e-security-class/sample}
\end{exampleblock}
\item 依存パッケージのインストール
\begin{exampleblock}{}
\$ \texttt{yarn install} or \texttt{npm install}
\end{exampleblock}
\item モックサーバの立ち上げ
\begin{exampleblock}{}
\$ \texttt{yarn start}
